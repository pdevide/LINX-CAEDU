...
*********** Comentado por Paulo Devidé -> 13-11-2014 (fim)
				...
				*****
				** CONTROLE DE VERBAS PARA PRODUTOS NACIONAIS
				** PAULO DEVIDE 30-11-15
				** somente produtos de origem nacional. ZORIGEM_PEDIDO_IMPORTADO() tem que retornar .F.
				**/
				** ALTERAÇÃO EM 10/05/16 --> MUDOU DE GERENTE PARA DIRETOR PARA AUTORIZAR CONTROLE DE SALDO DE OTB
				IF INLIST(ThisFormSet.p_Tool_Status,'I','A') ;
					AND !ZORIGEM_PEDIDO_IMPORTADO() AND ALLTRIM(V_COMPRAS_01.erp_cups_segmento) = 'VAREJO'
					IF NOT verifica_verba_OTB()
						** Pede senha e gera o Log
						lcMsg1 = "Saldo OTB Nacional para validação deste pedido é insuficiente!"
						IF !ZVALIDA_SENHA("DIRETOR",lcMsg1)
							RETURN .F.
						ELSE
							** Atualiza somente quando for autorizado pelo DIRETOR 
							Atualiza_LOG_Verba_OTB()
						ENDIF
					ENDIF
					...
					*** ATUALIZA CAMPOS DA TABELA COMPRAS_PRODUTO E PROSSEGUE COM A GRAVAÇÃO NORMAL
					SELECT V_COMPRAS_01_PRODUTOS 
					SCAN
				...
				ENDIF
				** (FIM) CONTROLE DE VERBAS PARA PRODUTOS NACIONAIS
				***************************************************
...
*** 
* Paulo Devidé - 01/12/2015
* Função é executada caso o saldo OTB esteja negativo e tenha a aprovação de um gerente
* Neste caso a operação autorizada é registrada
*/
FUNCTION Atualiza_LOG_Verba_OTB
**
** create table CAEDU_AUTORIZA_VERBA_OTB
** ( ID					int NOT NULL IDENTITY(1,1) PRIMARY KEY,
** PEDIDO				CHAR(8) NOT NULL,
** DATA_AUTORIZACAO		DATETIME NOT NULL,
** AUTORIZADO_POR		VARCHAR(25) NOT NULL,
** GRIFFE				char(25) NOT NULL,
** LINHA				char(25) NOT NULL,
** GRUPO				char(25) NOT NULL,
** SUBGRUPO				char(25) NOT NULL,
** MES					int	 NOT NULL,
** EMPENHO_AUTORIZADO 	NUMERIC(18,2) NULL )
	**SET STEP ON
	lcListaProdutos = ""
	SELECT curVerbas
	SCAN 					
	...
	TEXT TO lcSQL NOSHOW TEXTMERGE
		...
		WHERE PR.PRODUTO IN (<<lcListaProdutos>>)
	ENDTEXT
	f_select(lcSQL, "curGrupoProdutosOTB2")
	...
	from ;
		curverbas a ;
	inner join ;
		 CURGRUPOPRODUTOSOTB2 B ON B.PRODUTO=A.PRODUTO ;
	group by ;
		b.griffe,;
		b.linha,;
		b.grupo_produto,;
		b.subgrupo_produto ;
	INTO CURSOR CUR_TOTAL_AGRUPADO2

	SELECT CUR_TOTAL_AGRUPADO2
	SCAN 	

		TEXT TO lcSQL NOSHOW TEXTMERGE
		INSERT INTO CAEDU_AUTORIZA_VERBA_OTB (PEDIDO, DATA_AUTORIZACAO, AUTORIZADO_POR, 
		GRIFFE, LINHA, GRUPO, SUBGRUPO, MES, EMPENHO_AUTORIZADO)
		VALUES ('<<v_compras_01.PEDIDO>>', getdate(), '<<ALLTRIM(x_Diretor.USUARIO)>>', 
		'<<CUR_TOTAL_AGRUPADO2.GRIFFE>>', '<<CUR_TOTAL_AGRUPADO2.LINHA>>', '<<CUR_TOTAL_AGRUPADO2.grupo_produto>>', 
		'<<CUR_TOTAL_AGRUPADO2.subgrupo_produto>>', <<CUR_TOTAL_AGRUPADO2.erp_verbas_empenho_ano_mes>>, 
		<<CUR_TOTAL_AGRUPADO2.erp_verbas_empenho>>)
		ENDTEXT
