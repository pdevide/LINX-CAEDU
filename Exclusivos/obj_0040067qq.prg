				...
				** PAULO DEVIDE --> 09-12-2014 (INICIO)
				IF !llCancela
					...
					** Alimenta o campo memo ==> ERP_FOLLOW_UP_MARGEM
					...
					SELECT v_compras_01
					REPLACE v_compras_01.ERP_FOLLOW_UP_MARGEM WITH lcMsgFollowUp
...
ELSE
	...
	** localiza o preço de custo
	...
	ELSE && importado = 60% (margem minima)
...
FUNCTION ZVALIDA_SENHA(tcCargo, tcMensagem) 
	...
	frmLoginWindows.lblMensagem.Caption = tcMensagem	
	...
	**
	...
	WITH oExcel && objeto publico passado de parametro para esta função
		...
		*m.supp_ref = NVL(LOOKUP(cur_prop_compras.valor_propriedade,'00068',cur_prop_compras.propriedade),'') &&ALLTRIM(NVL(cur_produtos.REFER_FABRICANTE,''))
		m.supp_ref = ALLTRIM(NVL(cur_produtos.ERP_CUPS_STYLENUMBER,''))
...
*!*			m.cust_fob = CAST(m.cust_fob as numeric(14,2))
		...
		** m.shipment_date = NVL(LOOKUP(cur_prop_compras.valor_propriedade,'00029',cur_prop_compras.propriedade),'')
		m.shipment_date = DTOC(v_compras_01.ERP_CUPS_EMBARQUE_ATUAL)
...
FUNCTION zPedido_Excel_Atc
	...
	**
	...
	f_select("select * from produtos_precos where produto = ?tmpProdutos1.produto and codigo_tab_preco='40'","cur_preco_venda")
...
*!*			m.quantity_of_packs = NVL(LOOKUP(cur_prop_compras.valor_propriedade,'00032',cur_prop_compras.propriedade),0)
		...
		**-------------------------------------------------------**
		laBarCode[1,2] = v_compras_01_produtos.ERP_CUPS_PACKS_POR_CAIXA * vtotal_produto1.qtde_original 
		laBarCode[2,2] = vtotal_produto1.qtde_original
		laBarCode[3,2] = IIF(v_compras_01_produtos.ERP_CUPS_PACKS_POR_CAIXA>0,CEILING(vtotal_produto1.qtde_original / v_compras_01_produtos.ERP_CUPS_PACKS_POR_CAIXA),0)
...
DEFINE CLASS TformLoginWindows As Form
	...
	ADD OBJECT cmd1 As CommandButton WITH;
		Width=60, Height=25, Left=218, Top=150, ;
		Caption="Cancel" 
		
	ADD OBJECT cmd2 As CommandButton WITH;
		Width=60, Height=25, Left=284, Top=150, ;
		Caption="Ok", Default=.T.

	ADD OBJECT lblMensagem as Label WITH;
		Width=300, Height=50, Left=10, Top=90, Caption = "", Autosize = .F., Wordwrap = .T.
